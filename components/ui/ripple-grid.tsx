...
const RippleGrid: React.FC<RippleGridProps> = ({
  ...
  useEffect(() => {
    ...
    // Renderer creation with optimized DPR cap for light footprint
...
void main() {
    ...
    float aaWidth = 0.5;
    vec2 smoothB = smoothstep(vec2(0.0), vec2(aaWidth), b);

    vec3 color = vec3(0.0);
    color += exp(-gridThickness * smoothB.x * (0.8 + 0.5 * sin(PI * iTime)));
    color += exp(-gridThickness * smoothB.y);
    color += 0.5 * exp(-(gridThickness * 0.25) * sin(smoothB.x));
    color += 0.5 * exp(-(gridThickness * 0.333) * smoothB.y);
    ...
    // IntersectionObserver to pause animation when offscreen (lighter CPU/GPU usage)
