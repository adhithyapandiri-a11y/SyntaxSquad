  ...
  return (
    ...
    >
      <form onSubmit={handleSubmit} className="space-y-4 font-sans">
        ...
        <div>
          <div className="flex items-center justify-between mb-1">
            ...
            <button
              ...
              className="flex items-center gap-1.5 text-xs font-semibold text-[#0A0A0A] hover:underline disabled:opacity-50"
            >
        ...
        {aiResult && (
          <div className="p-4 rounded-xl bg-[#FAFAFA] border border-[rgba(0,0,0,0.06)] animate-in fade-in">
            <div className="flex items-center gap-2 mb-2 text-xs font-bold text-[#0A0A0A]">
              <Sparkles className="w-4 h-4 text-[#0A0A0A]" strokeWidth={1.5} />
              <span>AI Triage Analysis Complete</span>
            </div>
            <div className="grid grid-cols-2 gap-3 text-xs">
              <div>
                <span className="text-[#757575]">Auto Category:</span>
                <p className="font-semibold text-[#0A0A0A] capitalize">{category}</p>
              </div>
              <div>
                <span className="text-[#757575]">Assessed Urgency:</span>
                <p className="font-semibold text-[#0A0A0A] capitalize">{priority}</p>
              </div>
              <div>
                <span className="text-[#757575] flex items-center gap-1">
                  <Clock className="w-3 h-3 text-[#0A0A0A]" strokeWidth={1.5} /> Est. Fix Time:
                </span>
                <p className="font-semibold text-[#0A0A0A]">{aiResult.estimatedFixTime}</p>
              </div>
              <div>
                <span className="text-[#757575] flex items-center gap-1">
                  <Wrench className="w-3 h-3 text-[#0A0A0A]" strokeWidth={1.5} /> Assigned Team:
                </span>
                <p className="font-semibold text-[#0A0A0A]">{aiResult.suggestedStaff}</p>
              </div>
            </div>
          </div>
        )}
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <select
              ...
              className="w-full px-3.5 py-2.5 text-sm bg-[#FAFAFA] text-[#0A0A0A] border border-[rgba(0,0,0,0.06)] rounded-xl focus:outline-none capitalize"
            >
        ...
        <div>
          <label className="block text-xs font-bold uppercase tracking-wider text-[#757575] mb-1 flex items-center gap-1.5">
            <Upload className="w-3.5 h-3.5 text-[#0A0A0A]" strokeWidth={1.5} /> Photo Attachment (Optional)
          </label>
          <input
            type="url"
            value={photoUrl}
            ...
            className="w-full px-3.5 py-2 text-sm bg-[#FAFAFA] text-[#0A0A0A] border border-[rgba(0,0,0,0.06)] rounded-xl focus:outline-none"
          />
        </div>

        {/* Actions */}
        <div className="flex items-center justify-end gap-3 pt-4 border-t border-[rgba(0,0,0,0.06)]">
          <Button type="button" variant="ghost" onClick={onClose} className="text-[#0A0A0A] border border-[rgba(0,0,0,0.06)]">
            Cancel
          </Button>
          <Button type="submit" variant="primary">
            <Send className="w-4 h-4 mr-1.5 text-[#0A0A0A]" strokeWidth={1.5} />
            Submit Complaint
          </Button>
        </div>
      </form>
