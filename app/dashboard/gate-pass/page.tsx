...
export default function GatePassPage() {
  const { gatePasses, addGatePass, updateGatePassStatus, students } =
    useStore();

  const [isRequestModalOpen, setIsRequestModalOpen] = useState(false);
  const [selectedPassForQr, setSelectedPassForQr] = useState<GatePass | null>(
    null,
  );
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-[#757575] mt-1">
            Request out-passes, track warden approval status, and present
            scannable QR passes to gate security.
          </p>
        </div>
        ...
        >
          <Plus className="w-4 h-4 mr-1.5" strokeWidth={1.5} /> Request New Gate
          Pass
        </Button>
      </div>
          ...
          <Card key={pass.id} hoverEffect className="relative">
            <CardHeader>
              <div>
                ...
                <CardTitle className="text-base mt-2 tracking-tight text-[#0A0A0A]">
                  {pass.reason}
                </CardTitle>
              ...
              <div className="p-4 rounded-2xl bg-[#FAFAFA] border border-[rgba(0,0,0,0.04)] text-xs space-y-1.5">
                <div className="flex justify-between">
                  ...
                  <span className="font-bold text-[#0A0A0A]">
                    {pass.studentName} (Room {pass.roomNumber})
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-[#757575]">Leaving Date:</span>
                  <span className="font-medium text-[#0A0A0A]">
                    {formatDateTime(pass.leavingDate)}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-[#757575]">Expected Return:</span>
                  <span className="font-medium text-[#0A0A0A]">
                    {formatDateTime(pass.returnDate)}
                  </span>
                </div>
                <div className="flex justify-between">
              ...
              {pass.approvedByName && (
                <div className="flex items-center gap-1.5 text-xs text-[#0A0A0A] font-medium">
                  <CheckCircle2
                    ...
                    strokeWidth={1.5}
                  />{" "}
                  Approved by {pass.approvedByName}
                </div>
              )}
      ...
      {selectedPassForQr && (
        ...
        >
          <div className="flex flex-col items-center justify-center p-4 text-center space-y-4">
            ...
            <div>
              <p className="font-extrabold text-lg text-[#0A0A0A]">
                {selectedPassForQr.studentName}
              </p>
              <p className="text-xs text-[#757575]">
                Room {selectedPassForQr.roomNumber} •{" "}
                {selectedPassForQr.destination}
              </p>
              <p className="text-[11px] font-mono font-bold text-[#0A0A0A] mt-1">
                {selectedPassForQr.qrCodeHash}
              </p>
            </div>

            <div className="w-full p-4 rounded-2xl bg-[#FAFAFA] border border-[rgba(0,0,0,0.04)] text-xs text-[#0A0A0A]">
              ✓ Verified by{" "}
              {selectedPassForQr.approvedByName || "Hostel Warden"}
            </div>
          </div>
        </Modal>
      )}
