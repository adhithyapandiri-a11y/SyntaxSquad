      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            Digitized visitor entry logs with timestamp check-ins and security check-outs.
          </p>
        </div>

        <Button variant="primary" size="sm" onClick={() => setIsModalOpen(true)}>
          <Plus className="w-4 h-4 mr-1.5" /> Check-In New Visitor
        </Button>
      </div>
              ...
              {visitors.map((vis) => (
                <tr key={vis.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/40">
                  <td className="p-4">
                    ...
                    <p className="text-[10px] text-slate-500">{vis.visitorPhone}</p>
                  </td>
                  <td className="p-4">
                    <p className="font-bold text-slate-900 dark:text-white">{vis.studentName}</p>
                    <p className="text-[10px] text-blue-600 font-semibold">Room {vis.roomNumber}</p>
                  </td>
                  <td className="p-4">
                    <p className="font-semibold text-slate-800 dark:text-slate-200">{vis.relation}</p>
                    <p className="text-[10px] text-slate-500">{vis.purpose}</p>
                  </td>
                  <td className="p-4 text-slate-500">{formatDateTime(vis.timeIn)}</td>
                  <td className="p-4">
                    <Badge variant={vis.status === 'active' ? 'success' : 'outline'}>{vis.status}</Badge>
                  </td>
                  <td className="p-4 text-right">
                    {vis.status === 'active' ? (
                      <Button size="sm" variant="outline" onClick={() => checkOutVisitor(vis.id)}>
                        Check Out
                      </Button>
                    ) : (
                      <span className="text-[10px] text-slate-400">Out: {formatDateTime(vis.timeOut || '')}</span>
                    )}
                  </td>
                </tr>
              ))}
        ...
        >
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex justify-end gap-2 pt-3 border-t border-slate-100 dark:border-slate-800">
              ...
              <Button type="submit" variant="primary">Confirm Visitor Check-In</Button>
            </div>
          </form>
        </Modal>
      )}
