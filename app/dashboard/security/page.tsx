    ...
    <div className="space-y-6">
        ...
        <CardContent className="space-y-4">
          <form onSubmit={handleSimulateScan} className="flex gap-3">
            ...
            <Button type="submit" variant="primary" isLoading={isScanning}>
              <Search className="w-4 h-4 mr-1.5 text-[#0A0A0A]" strokeWidth={1.5} /> Scan QR Pass
            </Button>
          </form>
          ...
          {scanResult === 'not_found' && (
            <div className="p-4 rounded-xl bg-[#FAFAFA] border border-[rgba(0,0,0,0.06)] text-[#0A0A0A] flex items-center gap-3">
              <XCircle className="w-6 h-6 text-[#0A0A0A]" strokeWidth={1.5} flex-shrink-0 />
              <div>
                <p className="font-bold text-sm">Pass Not Found or Invalid Hash</p>
                <p className="text-xs text-[#757575]">The scanned QR hash does not match any approved gate pass in the database.</p>
              </div>
            </div>
          )}
          ...
          {scanResult && scanResult !== 'not_found' && (
            <div className="p-5 rounded-2xl bg-white border border-[rgba(0,0,0,0.06)] text-[#0A0A0A] space-y-3 animate-in fade-in shadow-[0_2px_8px_rgba(0,0,0,0.02)]">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <CheckCircle2 className="w-6 h-6 text-[#0A0A0A]" strokeWidth={1.5} />
              ...
              <div className="grid grid-cols-2 gap-4 text-xs">
                <div>
                  ...
                  <p className="font-bold text-sm text-[#0A0A0A]">{scanResult.studentName}</p>
                </div>
                <div>
                  <span className="text-[#757575]">Room Number:</span>
                  <p className="font-bold text-sm text-[#0A0A0A]">Room {scanResult.roomNumber}</p>
                </div>
                <div>
                  <span className="text-[#757575]">Reason & Destination:</span>
                  <p className="font-semibold text-[#0A0A0A]">{scanResult.reason} ({scanResult.destination})</p>
                </div>
                <div>
                  <span className="text-[#757575]">Approved By:</span>
                  <p className="font-semibold text-[#0A0A0A]">{scanResult.approvedByName}</p>
                </div>
              </div>
      ...
      <Card className="bg-white border border-[rgba(0,0,0,0.06)] rounded-2xl p-6 shadow-[0_2px_8px_rgba(0,0,0,0.02)] font-sans">
        ...
        <CardContent className="p-0 divide-y divide-[rgba(0,0,0,0.06)]">
          {visitors.map((v) => (
            <div key={v.id} className="p-4 flex items-center justify-between hover:bg-[#FAFAFA]">
              <div>
                <p className="text-sm font-bold text-[#0A0A0A]">{v.visitorName} ({v.relation})</p>
                <p className="text-xs text-[#757575]">Visiting {v.studentName} (Room {v.roomNumber}) • Phone: {v.visitorPhone}</p>
                <p className="text-[11px] text-[#757575] mt-0.5">Checked In: {formatDateTime(v.timeIn)}</p>
              </div>
              {v.status === 'active' ? (
