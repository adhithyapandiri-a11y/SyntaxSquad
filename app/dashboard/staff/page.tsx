...
export default function StaffDashboardPage() {
  ...
  return (
    <div className="space-y-6">
      {" "}
      {/* Staff Header */}{" "}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 p-6 rounded-2xl bg-white border border-[rgba(0,0,0,0.06)] shadow-[0_2px_8px_rgba(0,0,0,0.02)] text-[#0A0A0A] ">
        {" "}
        <div>
          {" "}
          <div className="flex items-center gap-2">
            {" "}
            <span className="px-2.5 py-0.5 rounded-full bg-[#FAFAFA] text-[#0A0A0A] text-xs font-bold uppercase">
              {" "}
              Maintenance Staff Workspace{" "}
            </span>{" "}
            <span className="text-xs text-[#757575]">
              Robert Vance • HVAC Lead
            </span>{" "}
          </div>{" "}
          <h1 className="text-2xl sm:text-3xl font-extrabold tracking-[-0.05em] mt-1">
            {" "}
            Assigned Work Orders & Repair Jobs{" "}
          </h1>{" "}
          <p className="text-sm text-[#757575] mt-1">
            {" "}
            Update work order status, upload repair proof photos, and resolve
            student complaints.{" "}
          </p>{" "}
        </div>{" "}
      </div>{" "}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <Card className="bg-white border border-[rgba(0,0,0,0.06)] rounded-2xl p-6">
          ...
          <CardHeader>
            {" "}
            <CardTitle className="flex items-center gap-2 text-[#0A0A0A] tracking-[-0.05em]">
              {" "}
              <Clock
              ...
              .map((cmp) => (
                <div key={cmp.id} className="p-5 space-y-3">
                  ...
                  <div className="flex items-start justify-between">
                    ...
                    <div>
                      {" "}
                      <span className="text-xs font-bold px-2 py-0.5 rounded bg-[rgba(0,0,0,0.04)] text-[#0A0A0A] capitalize">
                        {" "}
                        {cmp.category}{" "}
                      </span>{" "}
                      <h4 className="text-sm font-bold text-[#0A0A0A] mt-1">
                        {cmp.title}
                      </h4>{" "}
                      <p className="text-xs text-[#757575]">
                        Room {cmp.roomNumber} • Student: {cmp.studentName}
                      </p>{" "}
                    </div>{" "}
                    <Badge
                  ...
                  <p className="text-xs text-[#757575] ">{cmp.description}</p>{" "}
                  ...
                  <div className="flex items-center justify-end gap-2 pt-2 border-t border-[rgba(0,0,0,0.06)] ">
                    ...
                    <Button
                      ...
                      variant="outline"
                      onClick={() =>
                        updateComplaintStatus(cmp.id, "in_progress")
                      }
                    >
                      {" "}
                      Mark In Progress{" "}
            ...
            {complaints
              ...
              .map((cmp) => (
                <div key={cmp.id} className="p-4 space-y-2">
                  ...
                  <p className="text-xs text-[#757575]">
                    Room {cmp.roomNumber} • Resolved on{" "}
                    {formatDateTime(cmp.resolvedAt || cmp.createdAt)}
                  </p>{" "}
                  {cmp.resolutionNotes && (
                    <p className="text-xs text-[#0A0A0A] font-medium">
                      Notes: {cmp.resolutionNotes}
                    </p>
                  )}{" "}
                </div>
              ))}{" "}
          ...
          <form onSubmit={handleResolveSubmit} className="space-y-4">
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full px-3 py-2 text-sm bg-[#FAFAFA] border border-[rgba(0,0,0,0.06)] rounded-xl focus:outline-none "
              />{" "}
            </div>{" "}
            <div>
              {" "}
