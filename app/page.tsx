...
import { 
  Building2, Sparkles, ArrowRight, ShieldCheck, DoorOpen, 
  QrCode, CreditCard, Utensils, Check
} from 'lucide-react';
...
export default function LandingPage() {
  ...
  const features = [
    {
      icon: DoorOpen,
      title: 'Room Matrix',
      description: 'Visual floor-by-floor room grid with occupancy tracking, bed allocations, and instant vacancy filters.'
    },
    {
      icon: Sparkles,
      title: 'AI Triage',
      description: 'AI categorizes issues (HVAC, Plumbing), determines urgency, and predicts maintenance fix time instantly.'
    },
    {
      icon: QrCode,
      title: 'Gate Pass QR',
      description: 'Students request out-passes. Automated cryptographic QR code generation scannable by gate security.'
    },
    {
      icon: CreditCard,
      title: 'Fee Invoicing',
      description: 'Collect hostel rent and mess fees automatically with on-the-fly PDF receipts & revenue analytics.'
    },
    {
      icon: Utensils,
      title: 'Mess Planner',
      description: 'Dynamic 7-day meal schedules, calorie transparency, and daily meal attendance counters.'
    },
    {
      icon: ShieldCheck,
      title: 'Visitor Logs',
      description: 'Digitize visitor registers with instant check-in/out timestamps and emergency contact lookups.'
    }
  ];
  ...
  return (
    <div className="min-h-screen bg-white text-[#0A0A0A] font-sans selection:bg-[#0A0A0A] selection:text-white overflow-x-hidden">
      ...
      <header className={`fixed top-0 left-0 right-0 z-[100] h-[76px] transition-all duration-300 ${
        scrolled ? 'bg-white/80 backdrop-blur-md border-b border-[rgba(0,0,0,0.06)] shadow-[0_1px_2px_rgba(0,0,0,0.02)]' : 'bg-transparent'
      }`}>
        <div className="max-w-[1280px] mx-auto px-6 md:px-10 h-full flex items-center justify-between">
          <Link href="/" className="flex items-center gap-2">
            <span className="text-xl font-semibold tracking-tight">RoomZen</span>
            <span className="font-serif italic text-lg text-[#757575] mt-0.5">OS</span>
          </Link>
          ...
          <div className="flex items-center gap-4">
            <Link href="/login" className="hidden md:block text-[14px] font-medium text-[#757575] hover:text-[#0A0A0A] transition-colors">
              Sign In
            </Link>
      ...
      <section className="relative pt-[160px] pb-[160px] md:pb-[160px] min-h-screen flex flex-col items-center justify-center text-center px-6 md:px-10 overflow-hidden">
        ...
        >
          ...
          >
            The <span className="font-serif italic font-medium">smarter</span> way to manage hostels and PGs.
          </motion.h1>
          ...
          >
            Replace fragmented tools, spreadsheets, and WhatsApp groups with a single unified platform. Manage complaints, gate passes, room allocation, and fee payments instantly.
          </motion.p>
        ...
        >
          <div className="bg-[#FAFAFA] rounded-[20px] overflow-hidden border border-[rgba(0,0,0,0.04)] h-auto p-8 md:p-12">
            <div className="flex items-center justify-between border-b border-[rgba(0,0,0,0.06)] pb-8 mb-8">
              ...
              <div className="text-[12px] font-medium text-[#757575]">Overview</div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              {[
                ...
                <div key={i} className="p-6 bg-white rounded-2xl border border-[rgba(0,0,0,0.06)] shadow-[0_2px_8px_rgba(0,0,0,0.02)]">
                  <p className="text-[13px] text-[#757575] font-medium">{stat.label}</p>
                  <p className="text-[32px] font-semibold tracking-tight mt-2">{stat.value}</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-16 md:gap-24 items-center">
          ...
          <div>
            ...
            <div className="space-y-6">
              {[
                { title: 'Paper Registers', desc: 'Lost logs, impossible to search, and prone to damage.' },
                { title: 'Maintenance Chaos', desc: 'Issues buried in group chats with no tracking or accountability.' },
                { title: 'WhatsApp Spam', desc: 'Endless notifications that distract rather than organize.' }
              ].map((prob, i) => (
                <div key={i} className="p-8 bg-white border border-[rgba(0,0,0,0.06)] rounded-2xl">
                  <h3 className="text-[18px] font-semibold">{prob.title}</h3>
                  <p className="text-[15px] text-[#757575] mt-2 leading-relaxed">{prob.desc}</p>
                </div>
              ))}
            </div>
          </div>
      ...
      {/* Features Grid */}
      <section id="features" className="py-[100px] md:py-[160px] px-6 md:px-10 max-w-[1280px] mx-auto">
        <div className="mb-20">
          ...
          {features.map((f, i) => {
            ...
            return (
              ...
              >
                ...
                <p className="text-[15px] text-[#757575] leading-relaxed">{f.description}</p>
              </div>
            );
          })}
        </div>
      ...
      <section className="py-[100px] md:py-[160px] px-6 md:px-10 bg-[#0A0A0A] text-white">
        <div className="max-w-[1280px] mx-auto">
          ...
          <div className="space-y-12 border-t border-[rgba(255,255,255,0.1)] pt-12">
            {[
              { title: 'Grammar Polish', desc: 'Automatically cleans up student complaints for professional reading.' },
              { title: 'Smart Categorization', desc: 'Instantly routes HVAC, Plumbing, or Wi-Fi issues to the right staff.' },
              { title: 'Time Prediction', desc: 'Calculates urgency and estimates fix time based on historical data.' }
            ].map((ai, i) => (
              <div key={i} className="flex flex-col md:flex-row md:items-center gap-6 md:gap-16 pb-12 border-b border-[rgba(255,255,255,0.1)]">
                ...
                <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-16">
                  ...
                  <p className="text-[16px] text-[rgba(255,255,255,0.6)] leading-relaxed">{ai.desc}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <section className="py-[100px] md:py-[160px] px-6 md:px-10 max-w-[1280px] mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {[
            { quote: "RoomZen completely eliminated our paper registers. Security guards love the QR scanner, and students love not waiting in lines.", author: "Dr. Sarah Jenkins", role: "Hostel Director" },
            { quote: "The AI complaint triage is incredible. It assigns plumbing issues straight to the plumber without me playing middleman.", author: "Arthur Mitchell", role: "Chief Warden" },
            { quote: "Fee collection used to take weeks of spreadsheets. Now it's automated, and receipts generate instantly. A massive time saver.", author: "Priya Sharma", role: "Finance Head" }
          ].map((t, i) => (
            <div key={i} className="flex flex-col justify-between p-10 bg-[#FAFAFA] rounded-3xl border border-[rgba(0,0,0,0.04)]">
              <p className="text-[18px] text-[#0A0A0A] leading-relaxed mb-12">"{t.quote}"</p>
              <div>
      ...
      {/* Pricing */}
      <section id="pricing" className="py-[100px] md:py-[160px] px-6 md:px-10 max-w-[1280px] mx-auto border-t border-[rgba(0,0,0,0.06)]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="p-10 md:p-14 bg-white rounded-3xl border border-[rgba(0,0,0,0.06)] shadow-[0_8px_30px_rgb(0,0,0,0.04)] flex flex-col">
            ...
            <p className="text-[15px] text-[#757575] mt-2">Everything you need to run a modern hostel.</p>
            <div className="mt-10 mb-12">
              <span className="text-[64px] font-bold tracking-[-0.05em]">{billingCycle === 'annual' ? '₹5,999' : '₹6,999'}</span>
              <span className="text-[16px] text-[#757575] font-medium ml-2">/ month</span>
            </div>
            
            <div className="space-y-4 mb-12 flex-1">
              {['Up to 250 Student Beds', 'AI Complaint Assistant', 'Gate Pass & QR System', 'Automated Invoicing & PDF Receipts', 'Security Guard Gate Scanner'].map((feat, i) => (
          ...
          <div className="p-10 md:p-14 bg-[#FAFAFA] rounded-3xl border border-[rgba(0,0,0,0.06)] flex flex-col">
            ...
            <p className="text-[15px] text-[#757575] mt-2">For university campuses and massive residential chains.</p>
            <div className="mt-10 mb-12">
              <span className="text-[64px] font-bold tracking-[-0.05em]">Custom</span>
            </div>
            
            <div className="space-y-4 mb-12 flex-1">
              {['Unlimited Student Capacity', 'Custom Domain & Branding', 'Dedicated Account Manager', 'On-premise / SLA Support', 'Custom API Integrations'].map((feat, i) => (
                <div key={i} className="flex items-center gap-3">
                  <Check className="w-5 h-5 text-[#0A0A0A]" strokeWidth={2} />
                  <span className="text-[15px] text-[#757575]">{feat}</span>
                </div>
      ...
      {/* Footer */}
      <footer id="footer" className="bg-[#FAFAFA] border-t border-[rgba(0,0,0,0.06)] py-20 px-6 md:px-10">
        <div className="max-w-[1280px] mx-auto grid grid-cols-2 md:grid-cols-4 gap-12 md:gap-8">
          <div>
            <Link href="/" className="flex items-center gap-2 mb-6">
              <span className="text-[18px] font-semibold tracking-tight text-[#0A0A0A]">RoomZen</span>
            </Link>
            <p className="text-[13px] text-[#757575]">
              © 2026 RoomZen Inc.<br />All rights reserved.
            </p>
          </div>
          <div>
            <h4 className="text-[13px] font-semibold text-[#0A0A0A] mb-4">Product</h4>
            <ul className="space-y-3">
              <li><Link href="#features" className="text-[13px] text-[#757575] hover:text-[#0A0A0A] transition-colors">Features</Link></li>
              <li><Link href="#pricing" className="text-[13px] text-[#757575] hover:text-[#0A0A0A] transition-colors">Pricing</Link></li>
              <li><Link href="/dashboard/admin" className="text-[13px] text-[#757575] hover:text-[#0A0A0A] transition-colors">Live Demo</Link></li>
            </ul>
          </div>
          <div>
            <h4 className="text-[13px] font-semibold text-[#0A0A0A] mb-4">Resources</h4>
            <ul className="space-y-3">
              <li><Link href="/login" className="text-[13px] text-[#757575] hover:text-[#0A0A0A] transition-colors">Documentation</Link></li>
              <li><Link href="/login" className="text-[13px] text-[#757575] hover:text-[#0A0A0A] transition-colors">Help Center</Link></li>
              <li><Link href="/login" className="text-[13px] text-[#757575] hover:text-[#0A0A0A] transition-colors">API Reference</Link></li>
            </ul>
          </div>
          <div>
            <h4 className="text-[13px] font-semibold text-[#0A0A0A] mb-4">Company</h4>
            <ul className="space-y-3">
              <li><Link href="/login" className="text-[13px] text-[#757575] hover:text-[#0A0A0A] transition-colors">About</Link></li>
              <li><Link href="/login" className="text-[13px] text-[#757575] hover:text-[#0A0A0A] transition-colors">Privacy Policy</Link></li>
              <li><Link href="/login" className="text-[13px] text-[#757575] hover:text-[#0A0A0A] transition-colors">Terms of Service</Link></li>
            </ul>
          </div>
        </div>
      </footer>
